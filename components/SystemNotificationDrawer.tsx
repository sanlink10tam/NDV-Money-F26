...
const SystemNotificationDrawer: React.FC<SystemNotificationDrawerProps> = ({ 
  ...
  const showSpin = settings?.MAINTENANCE_MODE !== true;
  ...
  React.useEffect(() => {
    if (showNotif) {
      ...
      if ((!isDashboard || !showSpin) && activeTab === 'SPIN') {
        setActiveTab('NOTIF');
      } else {
  ...
  }, [initialTab, showNotif, isDashboard, showSpin]);
      ...
      <AnimatePresence mode="wait">
        {!isExpanded ? (
          ...
          >
            ...
            {/* Orange Badge */}
            {((showNotif && message) || (isDashboard && showSpin && (user.spins || 0) > 0)) && (
          ...
          >
            ...
            {/* Tabs */}
            {showNotif && isDashboard && showSpin && (
