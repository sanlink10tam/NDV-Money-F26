...
const LuckySpin: React.FC<LuckySpinProps> = ({ user, settings, onSpinResult, onClose }) => {
  ...
  const handleSpin = async () => {
    ...
    let totalWeight = eligiblePrizes.reduce((sum, p) => sum + p.effectiveWeight, 0);
    
    // Safety check: if no prizes are eligible (e.g. 100% win rate set but user meets no profit requirements)
    // We force a "Lose" result by giving weight to Lose segments
    if (totalWeight <= 0) {
      eligiblePrizes.forEach(p => {
        if (p.amount === 0) p.effectiveWeight = 1;
      });
      totalWeight = eligiblePrizes.reduce((sum, p) => sum + p.effectiveWeight, 0);
    }
    ...
    for (let i = 0; i < eligiblePrizes.length; i++) {
      if (eligiblePrizes[i].effectiveWeight > 0) {
