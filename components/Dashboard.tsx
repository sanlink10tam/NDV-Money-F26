...
const Dashboard: React.FC<DashboardProps> = React.memo(({ 
  ...
  const getStatusLabel = (status: string, settlementType?: string) => {
    switch (status) {
      case 'CHỜ DUYỆT':
      case 'ĐÃ DUYỆT':
      case 'ĐANG GIẢI NGÂN':
  ...
  const getStatusStyles = (status: string, isOverdue: boolean, settlementType?: string) => {
    ...
    switch (label) {
      case 'ĐANG XỬ LÝ': return 'text-amber-400';
                        ...
                        <div>
                          ...
                          <div className="flex items-center gap-1">
                            ...
                            <span className={`text-[7px] font-black uppercase flex items-center ${statusColor}`}>
                              {isOverdue ? 'QUÁ HẠN' : getStatusLabel(item.status)}
                              {item.status === 'CHỜ TẤT TOÁN' && (
                          ...
                          {(isOverdue || (currentFine > 0)) && (
                            <div className="flex flex-col items-end">
                              ...
                              <div className="bg-red-500/10 px-2 py-1 rounded-lg border border-red-500/20 shadow-[0_0_10px_rgba(239,68,68,0.1)]">
                                 <p className="text-[10px] font-black text-red-500 tracking-tighter">{currentFine.toLocaleString()} đ</p>
                              </div>
                            </div>
                          )}
                        </div>
